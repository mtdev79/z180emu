...
int take_interrupt(struct z180_state *cpustate, int irq)
{
	...
	if( irq == Z180_INT_TRAP )
	{
		PUSH(cpustate,  PC );
		cpustate->_PCD = 0x0;
		LOG("Z180 '%s' TRAP $0000\n", cpustate->device->m_tag);
		/* CALL opcode timing */
		cycles += cpustate->cc[Z180_TABLE_op][0xcd];  // ???
	}
	else if( irq == Z180_INT_NMI )
	{
		PUSH(cpustate,  PC );
	...
	else if( irq == Z180_INT_IRQ0 )
	{
		...
		{
			if (cpustate->device->m_type == Z180_TYPE_Z182 && (z80scc_device_z80daisy_irq_state(cpustate->device->z80scc)& Z80_DAISY_INT))
				irq_vector = z80scc_device_z80daisy_irq_ack(cpustate->device->z80scc);
			else
	...
	else
	{
		...
		PUSH(cpustate,  PC );
		RM16( cpustate, irq_vector, &cpustate->PC );
		LOG("Z180 '%s' INT%d [$%04x] = $%04x\n", cpustate->device->m_tag, irq, irq_vector, cpustate->_PCD);
		/* CALL opcode timing */
		cycles += cpustate->cc[Z180_TABLE_op][0xcd];
	}
