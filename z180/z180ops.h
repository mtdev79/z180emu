...
#define IN(cs,port)                                             \
	...
	(cs->device->m_type == Z180_TYPE_Z182 && (port & 0xff)>= Z182_REGSTART && (port & 0xff)<= Z182_REGEND))?  \
 ...
 ***************************************************************/
INLINE void z180_mmu(struct z180_state *cpustate)
{
 ...
 ***************************************************************/
#define RM(cs,addr) (cs)->memory->read_byte(cs, MMU_REMAP_ADDR(cs,addr))
 ...
 ***************************************************************/
#define WM(cs,addr,value) (cs)->memory->write_byte(cs, MMU_REMAP_ADDR(cs,addr),value)
#ifdef UNUSED_FUNCTION
 ...
 ***************************************************************/
INLINE void RM16( struct z180_state *cpustate, offs_t addr, union PAIR *r )
{
 ...
 ***************************************************************/
INLINE void WM16( struct z180_state *cpustate, offs_t addr, union PAIR *r )
{
 ...
 ***************************************************************/
INLINE UINT8 ROP(struct z180_state *cpustate)
{
	offs_t addr = cpustate->_PCD;
	cpustate->_PC++;
	return cpustate->memory->read_raw_byte(cpustate, MMU_REMAP_ADDR(cpustate, addr));
}
 ...
 ***************************************************************/
INLINE UINT8 ARG(struct z180_state *cpustate)
{
	offs_t addr = cpustate->_PCD;
	cpustate->_PC++;
	return cpustate->memory->read_raw_byte(cpustate, MMU_REMAP_ADDR(cpustate, addr));
...
INLINE UINT32 ARG16(struct z180_state *cpustate)
{
	offs_t addr = cpustate->_PCD;
	cpustate->_PC += 2;
	return cpustate->memory->read_raw_byte(cpustate, MMU_REMAP_ADDR(cpustate, addr)) | (cpustate->memory->read_raw_byte(cpustate, MMU_REMAP_ADDR(cpustate, addr+1)) << 8);
}
 ...
 ***************************************************************/
#define RETN    {                                               \
	LOG("Z180 '%s' RETN IFF1:%d IFF2:%d\n", cpustate->device->m_tag, cpustate->IFF1, cpustate->IFF2); \
	POP(cpustate, PC);                                                  \
 ...
 * INC  r8
 ***************************************************************/
INLINE UINT8 INC(struct z180_state *cpustate, UINT8 value)
{
	UINT8 res = value + 1;
 ...
 * DEC  r8
 ***************************************************************/
INLINE UINT8 DEC(struct z180_state *cpustate, UINT8 value)
{
	UINT8 res = value - 1;
 ...
 * RLC  r8
 ***************************************************************/
INLINE UINT8 RLC(struct z180_state *cpustate, UINT8 value)
{
	unsigned res = value;
 ...
 * RRC  r8
 ***************************************************************/
INLINE UINT8 RRC(struct z180_state *cpustate, UINT8 value)
{
	unsigned res = value;
 ...
 * RL   r8
 ***************************************************************/
INLINE UINT8 RL(struct z180_state *cpustate, UINT8 value)
{
	unsigned res = value;
	unsigned c = (res & 0x80) ? CF : 0;
 ...
 * RR   r8
 ***************************************************************/
INLINE UINT8 RR(struct z180_state *cpustate, UINT8 value)
{
	unsigned res = value;
	unsigned c = (res & 0x01) ? CF : 0;
 ...
 * SLA  r8
 ***************************************************************/
INLINE UINT8 SLA(struct z180_state *cpustate, UINT8 value)
{
	unsigned res = value;
	unsigned c = (res & 0x80) ? CF : 0;
	res = (res << 1) & 0xff;
 ...
 * SRA  r8
 ***************************************************************/
INLINE UINT8 SRA(struct z180_state *cpustate, UINT8 value)
{
	unsigned res = value;
	unsigned c = (res & 0x01) ? CF : 0;
 ...
 * SLL  r8
 ***************************************************************/
INLINE UINT8 SLL(struct z180_state *cpustate, UINT8 value)
{
	unsigned res = value;
	unsigned c = (res & 0x80) ? CF : 0;
	res = ((res << 1) | 0x01) & 0xff;
 ...
 * SRL  r8
 ***************************************************************/
INLINE UINT8 SRL(struct z180_state *cpustate, UINT8 value)
{
	unsigned res = value;
	unsigned c = (res & 0x01) ? CF : 0;
	res = (res >> 1) & 0xff;
