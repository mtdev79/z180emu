OP(illegal,2)
{
	logerror("Z180 '%s' ill. opcode $%02x $%02x\n",
			cpustate->device->m_tag, cpustate->memory->read_raw_byte(cpustate,(cpustate->_PCD-2)&0xffff), cpustate->memory->read_raw_byte(cpustate,(cpustate->_PCD-1)&0xffff));
	cpustate->int_pending[Z180_INT_TRAP] = 1;
	cpustate->IO_ITC |= Z180_ITC_TRAP;
	cpustate->IO_ITC |= Z180_ITC_UFO;
}
