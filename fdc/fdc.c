...
void fdc_error(fdc_t *fdc, int st5, int st6);
int timetolive;
int lastbyte=0;
...
fdc_write(uint16_t addr, uint8_t val, void *priv)
{
	...
	case 2:	/*DOR*/
		if (fdc->flags & FDC_FLAG_PCJR) {
			...
			/*if (!(val & 8) && (fdc->dor & 8)) { // disable DMA
				fdc->tc = 1;
				fdc_int(fdc);
			}*/
			if (!(val&4)) { // reset is asserted
				fdd_stop(real_drive(fdc, val & 3));
				fdc->stat = 0x00;
				fdc->pnum = fdc->ptot = 0;
			}
                        if (val&4) { // reset is clear (high)
				fdc->stat = 0x80;
				fdc->pnum = fdc->ptot = 0;
                        }
			if ((val&4) && !(fdc->dor&4)) { // reset released
				//timer_clock();
				fdc->time = 128LL; //* (1LL << TIMER_SHIFT);
				//timer_update_outstanding();
				fdc->interrupt = -1;
			...
			for (i = 0; i < FDD_NUM; i++) {
				...
				if ((!fdd_get_flags(drive_num)) || (drive_num >= FDD_NUM))
					...
					fdc_log("Set motoron D%d=%x\n",drive_num,(val & (0x10 << i)));
					motoron[drive_num] = (val & (0x10 << i));
				}
			}
	...
	case 5: /*Command register*/
		if ((fdc->stat & 0xf0) == 0xb0) { // PIO mode execution (data) write
			...
			if (fdc->pnum == 1) {
				...
				switch (fdc->interrupt & 0x1F) {
					...
					case 0xC: /*Read deleted data*/
						...
						fdc->st0 = real_drive(fdc, fdc->drive & 0x03);
						...
						if (!fdd_get_flags(drive_num) || (drive_num >= FDD_NUM) || !motoron[drive_num] || fdd_track0(drive_num)) {
							...
							{
								fdc_log("Already at track 0\n");
								fdc->st0 = 0x20 | (fdc->params[0] & 3);
							}
							fdc->pcn[fdc->params[0] & 3] = 0;
							fdc->interrupt = -3;
							//timer_clock();
							fdc->time = 1LL; //* (1LL << TIMER_SHIFT);
							//timer_update_outstanding();
							break;
						}
...
fdc_read(uint16_t addr, void *priv)
{
	...
	case 4: /*Status*/
		...
		break;
	case 5: /*Data*/
		if ((fdc->stat & 0xf0) == 0xf0) { // PIO mode execution (data) read
			fdc->stat &= ~0x80;
			if ((fdc->flags & FDC_FLAG_PCJR) || !fdc->fifo) {
				fdc->data_ready = 0;
				ret = fdc->dat;
		...
		if (fdc->paramstogo) {
			...
			fdc->paramstogo--;
			// reset INT, 8277A datasheet p.6
			if ((fdc->command & 0x1f) != 8 && fdc->fintr) { //avoid SIS race condition
				fdc->fintr = 0;
				fdc->int_state_cb(fdc,0);
			}
...
fdc_poll_common_finish(fdc_t *fdc, int compare, int st5)
{
    ...
    /*if (!(fdc->flags & FDC_FLAG_PS1))
	fdc->fintr = 0;*/
	//if (!fdc->dma)  // PIO last byte
	//	fdc->stat = 0xF0;
	//else
	//	fdc->stat = 0xD0;
    fdc->st0 = fdc->res[4] = (fdd_get_head(real_drive(fdc, fdc->drive)) ? 4 : 0) | real_drive(fdc, fdc->rw_drive);
    fdc->res[5] = st5;
    fdc->res[6] = 0;
    if (fdc->wrong_am) {
	fdc->res[6] |= 0x40;
	...
	if (!fdc->satisfying_sectors)
		fdc->res[6] |= 4;
	else if (fdc->satisfying_sectors == (fdc->params[5] << ((fdc->command & 0x80) ? 1 : 0)))
		fdc->res[6] |= 8;
    } else if (compare == 2) {
	...
	case 0x1D: /*Scan high or equal*/
		...
		if (fdc->sector == fdc->params[5]) {
			...
			if (!(fdc->command & 0x80)) {
				fdc->rw_track++;
				fdc->sector = 1;
				fdc_log("EOT MT=0 old_sector %d\n",old_sector);
				if (!(fdc->flags & FDC_FLAG_PCJR) && !fdc->tc /*&& fdc->dma && (old_sector == 255)*/)
	...
	case 0x0d: /*Format track*/
		if (fdc->format_state == 1) {
			...
			/*if (!(fdc->flags & FDC_FLAG_PS1))
				fdc->fintr = 0;*/
			fdc->stat = 0xD0;
			fdc->st0 = fdc->res[4] = (fdd_get_head(real_drive(fdc, fdc->drive)) ? 4 : 0) | real_drive(fdc, fdc->drive);
...
fdc_error(fdc_t *fdc, int st5, int st6)
{
	...
	//	fdc->stat = 0xD0;
	fdc->stat |= 0x80;
    fdc->st0 = fdc->res[4] = 0x40 | (fdd_get_head(real_drive(fdc, fdc->drive)) ? 4 : 0) | real_drive(fdc, fdc->rw_drive);
...
void
fdc_dma_ack(fdc_t *fdc) {
	fdc->dma_req_cb(fdc, 0);
}
...
fdc_data(fdc_t *fdc, uint8_t data)
{
	...
	static int state = 0;
    switch (state) {
        case 0: goto c0_fdc_data;
        case 1: goto c1_fdc_data;
    }
	c0_fdc_data:
	...
	//result = fdc->dma_channel_write(fdc,data);
 
	/* coroutine start */
	fdc_log("data coroutine start\n");
	state = 1;
	fdc->dma_buf = data;
	fdc->dma_req_cb(fdc, 1);
	return -0xff;
	
	/* coroutine resume */
	c1_fdc_data:
	fdc_log("data coroutine resume\n");
	state = 0;
...
int fdc_getdata(fdc_t *fdc, int last)
{
	...
	static int state = 0;
    switch (state) {
        case 0: goto c0_fdc_getdata;
        case 1: goto c1_fdc_getdata;
    }
	c0_fdc_getdata:
	...
	//data = fdc->dma_channel_read(fdc);

	/* coroutine start */
	fdc_log("getdata coroutine start\n");
	state = 1;
	fdc->dma_req_cb(fdc, 1);
	return -0xff;
	...
	/* coroutine resume */
	c1_fdc_getdata:
	fdc_log("getdata coroutine resume\n");
	state = 0;
...
fdc_t *
fdc_init(uint32_t flags, devcb_write_line int_state_cb, devcb_write_line dma_req_cb)
{
	...
	fdc->dma_ch = 2;*/
	fdc->int_state_cb =int_state_cb;
	fdc->dma_req_cb =dma_req_cb;
...
/*const fdc_type_t fdc_xt_device = {
...
const fdc_type_t fdc_xt_t1x00_device = {
    "PC/XT Floppy Drive Controller",
    0,
    FDC_FLAG_TOSHIBA,
    fdc_init,
...
const fdc_type_t fdc_pcjr_device = {
...
const fdc_type_t fdc_at_device = {
...
const fdc_type_t fdc_at_actlow_device = {
...
const fdc_type_t fdc_at_ps1_device = {
...
const fdc_type_t fdc_at_smc_device = {
...
const fdc_type_t fdc_at_winbond_device = {
...
const fdc_type_t fdc_at_nsc_device = {
