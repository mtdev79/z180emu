...
d86f_can_format(int drive)
{
    ...
    temp = temp && fdd_can_read_medium(drive);
...
d86f_get_bit(int drive, int side)
{
	...
	if (! surface_bit) {
		...
		if (current_bit) {
			...
			dev->last_word[side] |= 0/*(random_generate() & 1)*/;
		}
	}
    } else {
...
d86f_turbo_write(int drive, int side)
{
    ...
    dat = d86f_get_data(drive, 0);
	d86f_log("write turbo_pos: %d %02x\n",dev->turbo_pos,dat);
	if (dat == -0xff)
		return;
...
d86f_common_format(int drive, int side, int rate, uint8_t fill, int proxy)
{
	...
	dev->side_flags[side] |= (fdd_getrpm(drive) == 360) ? 0x20 : 0;
